  ...
  return (
    <>
      ...
      <aside className="position-fixed h-100 d-flex flex-column" style={{
        top: 0, left: 0, width: 260, paddingTop: 76, zIndex: 1030,
        background: '#ffffff',
        ...
        <div className="flex-grow-1 overflow-auto py-3">
          ...
          {links.map((link) => (
            <button key={link.label} onClick={() => go(link.path)}
              className="d-flex align-items-center gap-3 w-100 text-start border-0 px-3 py-3 mb-2"
              style={{ background: 'transparent', color: '#57534e', borderRadius: 12, fontSize: '1rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.background = 'var(--primary-light)'; e.currentTarget.style.color = 'var(--primary)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#57534e'; }}>
              <span style={{ color: 'var(--primary)', opacity: 0.8 }}>{link.icon}</span>
              {link.label}
            </button>
          ))}
        </div>
        <div className="p-4 border-top">
          <small style={{ fontSize: '0.8rem', color: '#a8a29e' }}>Scanexus v2.0</small>
        </div>
      </aside>
    </>
  );
